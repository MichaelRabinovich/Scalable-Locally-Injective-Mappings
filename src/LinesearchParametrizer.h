...
private:
  ...
  WOLF_COND_ERROR wolfe_conditions(const Eigen::MatrixXd& V, const Eigen::MatrixXi& F, Energy* energy,
    const Eigen::MatrixXd& d, double& alpha, Eigen::MatrixXd& uv,
   Eigen::MatrixXd& new_uv, double old_e, double& cur_e);
  ...
  const double wolfe_c1 = 1e-4;//1e-6;
  const double wolfe_c2 = 0.9;//0.999;
};
